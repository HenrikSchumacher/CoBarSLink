...
then you have to hunt down the file `omp.h` on your system. Then execute the following _Mathematica_ statement before one of the package's routines is run for the first time:
...
Then relaunch the _Mathematica_ kernel and run the package again. The build script will search these directories first (but after the system headers and libraries).
